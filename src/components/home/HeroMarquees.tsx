...
export default function HeroMarquees() {
  ...
  const tw = useTranslations("heroMarquees"); // nouvelles clés ci-dessous
  ...
  const trustPoints = [
    { icon: ShieldCheck, label: tw("trust1") },
    { icon: Clock, label: tw("trust2") },
    { icon: MessageSquareText, label: tw("trust3") },
    { icon: Star, label: tw("trust4") },
  ];

  // ⚠️ INCHANGÉ VOLONTAIREMENT — témoignages fictifs à gérer par toi-même
  const testimonials = [
  ...
  const paymentMethods = [tw("pay1"), tw("pay2"), tw("pay3"), tw("pay4")];
    ...
    <div className="space-y-6 py-8">
      <Marquee speedSeconds={35}>
        {services.map((key, i) => {
          ...
          return (
            <div key={key} className="flex items-center gap-2 bg-surface-container-lowest border border-outline-variant rounded-full px-4 py-2 flex-shrink-0">
      ...
      <Marquee speedSeconds={28} reverse>
        {trustPoints.map(({ icon: Icon, label }) => (
          <div key={label} className="flex items-center gap-2 bg-primary-container/10 rounded-full px-4 py-2 flex-shrink-0">
            <Icon size={16} className="text-primary" />
      ...
      <Marquee speedSeconds={40}>
        {testimonials.map((quote) => (
          <div key={quote} className="bg-surface-container-lowest border border-outline-variant rounded-xl px-5 py-3 flex-shrink-0 max-w-xs">
      ...
      <Marquee speedSeconds={25} reverse>
        {paymentMethods.map((method) => (
          <div key={method} className="flex items-center gap-2 bg-secondary-container/15 text-on-secondary-container rounded-full px-4 py-2 flex-shrink-0">

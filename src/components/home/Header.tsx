...
import logoTelephone from "../../../logotelephone.png";
      ...
      <nav className="max-w-container mx-auto px-4 md:px-8 h-16 flex items-center justify-between">
        <div className="flex items-center">
          <Link href="/" className="hidden md:flex items-center" aria-label="Ma Procédure">
            <Image src={logoPc} alt="Ma Procédure" width={150} height={42} priority className="h-10 w-auto" />
          </Link>
          <Link href="/" className="md:hidden flex items-center" aria-label="Ma Procédure">
            <Image src={logoTelephone} alt="Ma Procédure" width={110} height={32} priority className="h-8 w-auto" />
          </Link>
        </div>
        ...
        {/* Mobile : sélecteur de langue TOUJOURS visible + burger, côte à côte */}
        <div className="flex items-center gap-2 md:hidden">
          <Link
            href={pathname}
            locale={otherLocale}
            className="text-sm font-semibold px-3 py-1.5 rounded-full border border-outline-variant text-on-surface"
